...
message("Finding OpenCASCADE...")
#message("Searching for OpenCASCADE in: ${_incsearchpath}, apple? ${APPLE}, UNIX? ${UNIX}")
#set(OpenCascade_FOUND ON CACHE BOOL "OpenCascade found")
IF( NOT OpenCASCADE_FOUND STREQUAL TRUE )
#  message("Searching for OpenCASCADE in: ${_incsearchpath},!!!!")
  IF(UNIX)
    # set( _incsearchpath /opt/homebrew/Cellar/opencascade/7.9.2/include/opencascade/ /usr/include/opencascade /opt/occ/inc $ENV{CASROOT}/inc )
    set( _incsearchpath ${OPENCASCADE_PATH}/include/ opencascade/ /usr/include/opencascade /opt/occ/inc $ENV{CASROOT}/inc )
    message("Searching for OpenCASCADE in: ${_incsearchpath}, apple? ${APPLE}")
    if (APPLE)
      set( _testlibname libTKernel.dylib )
    else (APPLE)
      set( _testlibname libTKernel.so )
    endif (APPLE)
    # set( _libsearchpath /opt/homebrew/Cellar/opencascade/7.9.2/lib/ /usr/lib /opt/occ/lib $ENV{CASROOT}/lib )
    set( _libsearchpath ${OPENCASCADE_PATH}/lib/ /usr/lib /opt/occ/lib $ENV{CASROOT}/lib )
    set(OpenCascade_FOUND ON CACHE BOOL "OpenCascade found")
  ELSE(UNIX)
    IF (WIN32)

...
static gsGeometry<real_t>* occtBSplineCurveToGs(const Handle(Geom_BSplineCurve)& c, real_t scale = 1.0)
{
    ...
    // OCCT knots / mults
    const int nbKnots = c->NbKnots();
    std::vector<real_t> kv;
    kv.reserve(nbKnots + 2 * deg + 2); // rough reserve

    for (int i = 1; i <= nbKnots; ++i)
    {
        const real_t knot = (real_t)c->Knot(i);
        const int mult = c->Multiplicity(i);
        for (int k = 0; k < mult; ++k)
            kv.push_back(knot);
    }

    // Build G+Smo basis from a full knot vector
    // gsKnotVector<real_t> KV(kv.begin(), kv.end());
    gsKnotVector<real_t> KV(kv);
    gsBSplineBasis<real_t> basis(KV, deg);
    // gsNurbsBasis<real_t> basis(KV, deg);

    // Control points / weights
    const bool rational = c->IsRational();
    const int geoDim = 3;
    const int rows = rational ? (geoDim + 1) : geoDim;

    gsMatrix<real_t> coefs(rows, nbPoles);
    for (int i = 1; i <= nbPoles; ++i)
    {
        const gp_Pnt P = c->Pole(i);
        coefs(0, i - 1) = scale * (real_t)P.X();
        coefs(1, i - 1) = scale * (real_t)P.Y();
        coefs(2, i - 1) = scale * (real_t)P.Z();

        if (rational)
            coefs(3, i - 1) = (real_t)c->Weight(i);
    }

    // NURBS curve
    // return new gsNurbs<real_t>(basis, coefs);
    // return new gsNurbs<real_t>(basis, coefs);
    // TODO ignore rational for now
    return new gsBSpline<real_t>(basis, coefs.transpose());
}

static gsGeometry<real_t>* occtBSplineSurfaceToGs(const Handle(Geom_BSplineSurface)& s, real_t scale = 1.0)
{
    if (s.IsNull()) return nullptr;

    const int p = s->UDegree();
    const int q = s->VDegree();

    const int nU = s->NbUPoles();
    const int nV = s->NbVPoles();
    const int nCtrl = nU * nV;

    // Knots U
    const int nbUKnots = s->NbUKnots();
    std::vector<real_t> ku;
    for (int i = 1; i <= nbUKnots; ++i)
    {
        const real_t knot = (real_t)s->UKnot(i);
        const int mult = s->UMultiplicity(i);
        for (int k = 0; k < mult; ++k)
            ku.push_back(knot);
    }

    // Knots V
    const int nbVKnots = s->NbVKnots();
    std::vector<real_t> kv;
    for (int i = 1; i <= nbVKnots; ++i)
    {
        const real_t knot = (real_t)s->VKnot(i);
        const int mult = s->VMultiplicity(i);
        for (int k = 0; k < mult; ++k)
            kv.push_back(knot);
    }

    gsKnotVector<real_t> KU(ku);
    gsKnotVector<real_t> KV(kv);

    // gsBSplineBasis<real_t> bu(KU, p);
    // gsBSplineBasis<real_t> bv(KV, q);
    gsTensorBSplineBasis<2, real_t> basis(KU, KV);

    // Control points / weights
    const bool rational = s->IsURational() || s->IsVRational(); // usually consistent
    const int geoDim = 3;
    const int rows = rational ? (geoDim + 1) : geoDim;

    gsMatrix<real_t> coefs(rows, nCtrl);

    // OCCT pole indexing is (iu=1..nU, iv=1..nV)
    // We map to a linear index. Important: choose a consistent ordering.
    // Here: iv-major (v outer), then iu (u inner): idx = (iv-1)*nU + (iu-1)
    for (int iv = 1; iv <= nV; ++iv)
    {
        for (int iu = 1; iu <= nU; ++iu)
        {
            const int idx = (iv - 1) * nU + (iu - 1);
            const gp_Pnt P = s->Pole(iu, iv);

            coefs(0, idx) = scale * (real_t)P.X();
            coefs(1, idx) = scale * (real_t)P.Y();
            coefs(2, idx) = scale * (real_t)P.Z();

            if (rational)
                coefs(3, idx) = (real_t)s->Weight(iu, iv);
        }
    }

    // return new gsNurbs<real_t>(basis, coefs);
    // TODO ignore rational for now
    return new gsTensorBSpline<2, real_t>(basis, coefs.transpose());
}

// Peel “trim wrappers” (Rhino exports often produce trimmed entities)
static Handle(Geom_Surface) unwrapSurface(const Handle(Geom_Surface)& s)
{
    if (s.IsNull()) return s;
    Handle(Geom_RectangularTrimmedSurface) rt = Handle(Geom_RectangularTrimmedSurface)::DownCast(s);
    if (!rt.IsNull())
        return rt->BasisSurface();
    return s;
}

static Handle(Geom_Curve) unwrapCurve(const Handle(Geom_Curve)& c)
{
    if (c.IsNull()) return c;
    Handle(Geom_TrimmedCurve) tc = Handle(Geom_TrimmedCurve)::DownCast(c);
    if (!tc.IsNull())
        return tc->BasisCurve();
    return c;
}

// ------------------------------------
// Main: Read Rhino IGES -> gsMultiPatch
// ------------------------------------
                    ...
                    real_t scale)
{
    ...
    const IFSelect_ReturnStatus stat = reader.ReadFile(igsFile.c_str());
    if (stat != IFSelect_RetDone)
    {
        std::cerr << "[readOcctIgesMp] IGES read failed: " << igsFile << "\n";
        return false;
    }

    // Transfer into OCCT shape
    reader.TransferRoots();
    TopoDS_Shape shape = reader.OneShape();
    if (shape.IsNull())
    {
        std::cerr << "[readOcctIgesMp] Transfer produced null shape.\n";
        return false;
    }
    ...
    for (int i = 1; i <= faceMap.Extent(); ++i)
    {
        ...
        surf = unwrapSurface(surf);

        Handle(Geom_BSplineSurface) bs = Handle(Geom_BSplineSurface)::DownCast(surf);
        if (!bs.IsNull())
        {
            if (gsGeometry<real_t>* g = occtBSplineSurfaceToGs(bs, scale))
                mp.addPatch(*g);
        }
        else
        {
            // If Rhino exported analytic surfaces, OCCT may store them as Geom_Plane/Cylinder/etc.
            // You can optionally convert them to BSpline via GeomConvert if you want.
            // For now we skip non-BSpline surfaces.
        }
    }
    ...
    if (importCurves)
    {
        ...
        for (int i = 1; i <= edgeMap.Extent(); ++i)
        {
            ...
            if (!bc.IsNull())
            {
                if (gsGeometry<real_t>* g = occtBSplineCurveToGs(bc, scale))
                    mp.addPatch(*g);
            }
            else
            {
                // Same note: analytic curves (lines, circles) may appear; convert if needed.
            }
        }
    }

    return mp.nPatches() > 0;
}
